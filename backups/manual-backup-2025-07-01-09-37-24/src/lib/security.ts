...
interface SecurityEvent {
  ...
  class SecurityMonitor {
    ...
    logEvent(event: Omit<SecurityEvent, 'timestamp'>) {
      ...
      console.log(`[SECURITY] ${event.type}: ${event.ip} - ${JSON.stringify(event.details || {})}`)
    ...
    private checkForSuspiciousActivity(ip: string) {
      ...
      if (recentEvents.length > 20) {
        console.warn(`[SECURITY ALERT] Suspicious activity from ${ip}: ${recentEvents.length} events in 5 minutes`)
